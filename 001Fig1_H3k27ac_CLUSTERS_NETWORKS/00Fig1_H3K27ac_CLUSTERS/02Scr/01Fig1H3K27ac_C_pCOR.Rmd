...
# **Objective**
-To perform unsupervised cluster analysis from H3K27ac ChIP-seq during Zebrafish heart regeneration.
...
library(knitr)
knitr::include_graphics("./SUMMARY_FIG1.png")
```
...
PTHA11="../03OUTPUT/"
dir.create(PTHA11)
PROJECT="01Fig1H3K27ac_C_pCOR"

PTHA=paste(PTHA11,PROJECT,"/",sep="")
PORT=paste(PTHA,"forNGS/",sep="")
PORT2=paste(PTHA,"forMOTIFs/",sep="") 
PORT3a=paste(PTHA,"forProfR/",sep="")

PORT3=paste(PTHA,"forProfR/","BED_CL/",sep="")
PORT4=paste(PTHA,"forProfR/","TSS_CL/",sep="") 
dir.create(PTHA)
dir.create(PORT)
dir.create(PORT2)
dir.create(PORT3a)
...
# Import Input Data 
## Re-analyzed data of bulk RNA_seq matrix from PRJNA509429
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
meta = "../../../000MASTER_Input_FILES/02RE_analysis_PRJNA509429_CLUS.csv"
...
## Load the merge H3K27ac ChIP-seq peak files
...
PEAKS=unzip("../01DATA/001ANNO_PEAKS_H3K27ac_ZF_ALL_x_BEDTO_SE.zip")
...
## Load the count matrix of H3K27ac peaks
```{r,message=FALSE,class.source="bg-info",warning=FALSE}
...
# Normalization of the H3K27ac ChIP-seq peaks
...
## Merge Histone data with RNA-seq data
```{r,message=FALSE,class.source="bg-info",warning=FALSE}
...
## Filter correlated genes to H3K27ac peaks
...
## Code for barplot of the correlation by genomic region
...
pdf(file=paste(PTHA,"001_", PROJECT,"_COR_REG",".pdf", sep=""),width=5, height=4)  
print(P3)
print(P2)
...
## Barplot from Correlation of H3K27ac to Expressed genes
...
pdf(file=paste(PTHA,"001_FIGS1B_",PROJECT,"_COR_G",".pdf", sep=""),width=5, height=4)  
...
#col<- colorRampPalette(c(upcol, "white", downcol))(20)
...
## Code to perform the line plot in Fig1C
```{r,fig.dim = c(7, 8),message=FALSE,class.source="bg-info",warning=FALSE,echo=T}
XL<- "dpci"
YL="zscoreLog2_meanRPKM+1"
NAME55<- c("00","01","04","14","45")
## THANK YOU JESUS!!
...
## Print FIG1B Clusters of H3K27ac
```{r,fig.dim = c(6, 8),class.source="bg-info"}
print(arrange0b) 
#print(arrange0b1)
```

# Handle the output Matrixes
...
## Code to perform barplots for the genomic region 
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
...
# Barplots to characterized the clusters
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=FALSE}
XL="dpci"
YL="Peak distribu (%)"
YL2="Peak group"
...
## Plots vertical from SE and Typical enhancer (TYE) across clusters
...
# Export matrixes of Peaks files
## Export Peak file in Bed format for MOTIFS search
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=T}
DIS_TT<- list()
...
## Peaks bed files for NGSPLot program
- we export the EMSEMBL; it was not used in this manuscript. However, the script produce these files to potentially used for other experiments.
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=T}
...
## Peaks to use in ProfileR 

```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=T}
upcol<- "#d5648b" # magenta from RdBu
nc<- "#bdbdbd" # gray
downcol<- "#79d0d9" # green from RdBu
PEAKS="../../../000MASTER_Input_FILES/02TSS_ZF_DanRer11_NON/001Emsem_danRer11_TSS2kb_Annot.bed"
...
rm("./001ANNO_PEAKS_H3K27ac_ZF_ALL_x_BEDTO_SE.bed")
...
## Barplot of correlation by Clusters
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=FALSE,fig.keep='none'}
CO22_COR <-  c("darkblue","grey","gold","#4393C3","brown", "black")
YL3<- "distr of peaks (%)"
...
## Barplot of Correlation of peak by cluster by genome area (Promoter, Genebody and Intergenic
