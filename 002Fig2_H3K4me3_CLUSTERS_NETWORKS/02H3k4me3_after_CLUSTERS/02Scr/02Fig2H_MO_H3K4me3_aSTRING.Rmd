...
##### To integrates the Protein network from the selected H2K4me3 clusters.

## **Input files**
   ##### k4m3_Clu1_Protein_string_interactions_short.tsv
   ##### k4m3_Clu2_Protein_string_interactions_short.tsv		
   ##### k4m3_Clu4_Protein_string_interactions_short.tsv
   ##### k4m3_Clu6_Protein_string_interactions_short.tsv
      - these files are the output file from the protein network from String web online tool.
        We used all the Proteins associated with the enriched DNA motifs with a log10Pvalue>=1

## **Output files**
    #### 001_FIG1E_02Fig2H_MO_H3K4me3_aSTRING_HEAT_TOP10.pdf
        - Heatmap used in the main figure

#### 001_FIG1E_02Fig2H_MO_H3K4me3_aSTRING_SUPLE.pdf
        - Bar plot to have an Idea of how specific the proteins among the clusters

#### 00forCytoScape
        - Contains all the matrixes to load in Cytoscape (v 3.6.1) to visualize the Protein networks.
...
### Heatmap from Interactome proteins selected based in the TOP 10 interactome afer the String 
...
### Barplot as suplementary figure in the Script. It contains top10 proteins per cluster
...
### Main TF selected to show as a network in Cytoscape version 3.6.1
```{r,message=FALSE,class.source="bg-info",warning=FALSE}
PROT1="GATA4"
PROT2="ETS1"
PROT3="CDX2"
...
PORT2<- paste(PORT,"00forCytoScape","/",sep="")
dir.create(PORT2)
...
CY="forCyto2"
write.table(MA7_25_TOP10_1,file=paste(PORT2, PROJECT,"_",NAME_G2[1,3],"_",CY,".txt", sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
write.table(MA7_25_TOP10_2,file=paste(PORT2, PROJECT,"_",NAME_G2[2,3],TFC01,"_",CY,".txt", sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
write.table(MA7_25_TOP10_3,file=paste(PORT2, PROJECT,"_",NAME_G2[3,3],U2,"_",CY,".txt", sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
write.table(MA7_25_TOP10_3,file=paste(PORT2, PROJECT,"_",NAME_G2[4,3],"_",U31,"_",CY,".txt", sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)

TOGe<- rbind(MA7_25_TOP10_1,MA7_25_TOP10_2,MA7_25_TOP10_3,MA7_25_TOP10_4)

write.table(TOGe,file=paste(PORT2,"002_FIG1F_", PROJECT,"_MAT","_",CY,".txt",sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
write.xlsx(HHa,file=paste(PORT2,"002_FIG1F_", PROJECT,"_MAT",".xlsx",sep=""),rowNames=T)
