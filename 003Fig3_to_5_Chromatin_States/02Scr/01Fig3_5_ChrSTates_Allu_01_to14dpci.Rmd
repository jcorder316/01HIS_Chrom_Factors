---
title: "01Fig3_5_ChrSTates_Allu_01_to14dpci"
...
##### To define all the chromatin states based on the marks from the 4 histone marks used in this study.
##### To generate all the peaks files for Transcriptional factors in Neighborhood ZONE(TFinZONE) in all of teh stages as reference (01dcpi, 04dcpi and 14dcpi)

## **Input files**
   ##### COMBI_META_ALL_COMBINA.csv
      - Quantification from the enrichment of all histone marks used in this study.
   ##### META_ZF_heart_re_HISTONTES.txt	
      - Metadata from all histone marks used in this study.
	
   ##### 001ZF_Geno_Areas_bedto_HEATMAP_MAT_ALL.txt
      - All the H3K4me3 peaks summarised in this manuscript
   ##### ENH_BIVA_EM_REF_NON_OVER_ENH_ALL_for_forUCSC.bed
      - All H3K4me3 peaks annotated.

   ##### MAT_all_HM_Zf_danRer11_BWAVE_A.txt
      - Quantification from the enrichment of all histone marks used in this study.
...
PTHA1="../03OUTPUT/"
dir.create(PTHA1)
PROJECT="01Fig3_5_ChrSTates_Allu_01_to14dpci"

PTHA=paste(PTHA1,PROJECT,"/",sep="")
PTHAa<- paste(PTHA,"00ALLU_PLOTS","/",sep="")
PTHA_1=paste(PTHA,"01MAT_fALLU/",sep="")
PTHA_2=paste(PTHA,"03GOs_Stage/",sep="")
PTHA_3=paste(PTHA,"01for_MOTIFs/",sep="")
...
### set the number of cores to work. This will be accorning to your PC
...
### Calculate the mean value per Histone mark ######
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
DE_list <- list(NULL)
DE_list1 <- list(NULL)
...
```
## To CALCULTE  each Histone mark Q1 value
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
...
## Overview of how the Chromatin States are looking during ZF HR
```{r,fig.dim = c(6, 5),message=FALSE,class.source="bg-info",warning=FALSE}

print(ap1b)
```
########## ALLUVIAL ####### and HEATMAPS #############
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
...
############ check with heatmap to get an idea of the dynamics of the Histone marks  ################
...
hcm = HeatmapAnnotation(df = data.frame(type = type),col = list(type = c(X00dpci= "grey",X01dpci= "#AA4499",X04dpci= "deeppink",X14dpci= "violet",X45dpci= "darkgreen")))
L=-1
M=1
clu<- mat_cm_TMM$ENH
...
KM=5
#### FOR VOLCANO
upcol<- "#d6604d" # magenta from PiyG
nc<- "#000000" # black
...
### To USE Heatmap only by Peak Overlap poorly Explain the dynamics of Histones during ZF HR 
...
#write.xlsx(MA5_total,file=paste(PTHA,"006_",PROJECT,TRY,"_from_HEATMAP.xlsx",sep=""),overwrite = T)
```


##### Preparing the Matrix alluvial plot representation. Loop to write in each column that contains "1". The name of the Chromatin States determined
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
TY="2_OR_MORE"
...
#head(CHECK068)
## Here we need to merge the peaks by PEAK_ID2 in order to make the dynamic of the PEAK in one row
```
...
## Idendify Divergent areas based of the 04dpci as point of reference 
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
############################## Activation (Ea or Pa) to Repression (Ep, Er,Epr, BiV, Ps)  #######################################
...
## Idendify Divergent areas based of the 14dpci as point of reference 
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
############################## Activation (Ea or Pa) to Repression (Ep, Er,Epr, BiV, Ps)  #######################################
...
```
### For Motif search 01dpci to use for MOAC
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
DCPI="_01dcpi_"
...
```
### For Motif search 04dpci to use for MOAC
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
DCPI2="_04dcpi_"
...
### For Motif search 14dpci to use for MOAC
```{r,class.source="bg-info",echo=TRUE,warning=FALSE}
DCPI3="_14dcpi_"
...
### Alluvial plots from all Divergent Areas and ALL Chromatin States
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=FALSE}
...
pdf(file=paste(PTHAa,"001_",PROJECT ,"_",".pdf",sep=""), width=7, height=5) 
...
dev.off()
```
### Alluvial plots from Figures S3 ALl Chromatin Stages
...
pdf(file=paste(PTHA,"001",PROJECT,"_3_SELEC1_POIS.pdf", sep=""),width=5, height=4)  
{
  ...
  dev.off()}
```
### Barplots to check the dynamic of Chromatin states per stage
...
YL2="number of Peaks"
MI=3
pdf(file=paste(PTHA,"002_",PROJECT,"_3_SELEC1_by_GENOMICS_2.pdf", sep=""),width=10, height=3)  
...
## Chromatin States distribution showed in bar plot
...
## Genomic annotation of the Chromatin States
...
{pdf(file=paste(PTHA,"004_",PROJECT,"_3_SELEC1_by_GENOMICS_v_CLUS_K27ac.pdf", sep=""),width=5, height=10)  
...
## Chromatin States by H3K27ac Clusters
```{r,fig.dim = c(10, 10),message=FALSE,class.source="bg-info",warning=FALSE}
...
## Chromatin States ONLY NON-Annotated PEAKS (NF). They might be potential NOvels genes
```{r,fig.dim = c(5, 7),message=FALSE,class.source="bg-info",warning=FALSE}
grid.arrange(  P2,P3, ncol = 1,nrow = 2)
```

## GO analysis by Chro States at 04dpci as Example time point
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=FALSE}
TRY="2_OR_MORE"
PP="fdr"
PP2="fdr"
...
GO00b<- dotplot(formulA_IaesddBGI11FC1_25_1_5_FDR,showCategory=3, title=paste(PP,MIO,sep="_"))
...
GO04_1<- ggplot(ewp_1@result[1:10,], aes(x=reorder(Description, FDR2),FDR2,  fill=FDR2)) +  geom_bar(stat = "identity") +   coord_flip() +scale_fill_continuous(low="blue", high="deeppink") +  labs(x = "", y = "", fill = "FDR2",title = paste(PP,MIO,NAME_ENH[1,1],sep="_"))+blank_them2
 
ewp_2@result$FDR2<- -log10(ewp_2@result$pvalue )
GO04_2<- ggplot(ewp_2@result[1:10,], aes(x=reorder(Description, FDR2),FDR2,  fill=FDR2)) +    geom_bar(stat = "identity") + coord_flip() +scale_fill_continuous(low="blue", high="deeppink") +  labs(x = "", y = "", fill = "FDR2",title = paste(PP,MIO,NAME_ENH[2,1],sep="_")) +blank_them2

ewp_3@result$FDR2<- -log10(ewp_3@result$pvalue )
GO04_3<- ggplot(ewp_3@result[1:10,], aes(x=reorder(Description, FDR2),FDR2,  fill=FDR2)) +  geom_bar(stat = "identity") + coord_flip() +scale_fill_continuous(low="blue", high="deeppink") +  labs(x = "", y = "", fill = "FDR2",title = paste(PP,MIO,NAME_ENH[3,1],sep="_")) + blank_them2

ewp_4@result$FDR2<- -log10(ewp_4@result$pvalue )
GO04_4<- ggplot(ewp_4@result[1:10,], aes(x=reorder(Description, FDR2),FDR2,  fill=FDR2)) +  geom_bar(stat = "identity") + coord_flip() +scale_fill_continuous(low="blue", high="deeppink") +  labs(x = "", y = "", fill = "FDR2",title = paste(PP,MIO,NAME_ENH[4,1],sep="_")) +blank_them2

ewp_5@result$FDR2<- -log10(ewp_5@result$pvalue )
GO04_5<- ggplot(ewp_5@result[1:10,], aes(x=reorder(Description, FDR2),FDR2,  fill=FDR2)) +  geom_bar(stat = "identity") + coord_flip() +scale_fill_continuous(low="blue", high="deeppink") +  labs(x = "", y = "", fill = "FDR2",title = paste(PP,MIO,NAME_ENH[5,1],sep="_")) + blank_them2

{pdf(file=paste(PTHA_2,"001_",PROJECT, "_GO_",PP,MIO,TRY, ".pdf",sep=""), width=10, height=7)  
print(GO00b1)
print(GO04_1)
print(GO04_2)
print(GO04_3)
...
egoFDR2<- subset(formulA_IaesddBGI11FC1_25_1_5_FDR@compareClusterResult, ID=" ", select=c("Cluster","ID","pvalue", "p.adjust","GeneRatio", "Description","geneID"))
egoFDR3_1<- subset(ewp_1@result, ID=" ", select=NAME_GO)
egoFDR3_2<- subset(ewp_2@result, ID=" ", select=NAME_GO)
egoFDR3_3<- subset(ewp_3@result, ID=" ", select=NAME_GO)
egoFDR3_4<- subset(ewp_4@result, ID=" ", select=NAME_GO)
egoFDR3_5<- subset(ewp_5@result, ID=" ", select=NAME_GO)
...
write.xlsx(egoFDR2,file=paste(PTHA_2,"001_",PROJECT,"_",PP,MIO,TRY,'_MATRIX_2.xlsx',sep=""),overwrite =T)
write.table(egoFDR2,file=paste(PTHA_2,"001_",PROJECT,"_",PP,MIO,TRY,"_MATRIX_2.txt",sep=""),sep="\t",row.names = F,col.names=T,dec=".")
write.xlsx(egoFDR3diff2,file=paste(PTHA_2,"001",PROJECT,"_",PP,MIO,TRY,"_forREVIGO2.xls",sep=""),overwrite =T)
write.table(egoFDR3diff2,file=paste(PTHA_2,"001",PROJECT,"_",PP,MIO,TRY,"_forREVIGO.txt",sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
```
