...
## **Input files**
   ...
   ##### 08TSS_Enh_w_14dpci_fHeatmap_ALL.xlsx
...
PTHA="../03OUTPUT/"
dir.create(PTHA)
PROJECT="03a_TFinZONE_at_14dpci"
...
#WORK11A11="../01DATA_ORI/07aMO_forMOAC/08TSS_Enh_w_14dpci_fHeatmap_ALL.xlsx"
...
#length(MAT_MO1$SYMBOL2)
#table(MAT_MO1$GROUP)

#00A_R_14dpci  01R_A_14dpci 02U_Ea_14dpci  03u_R_14dpci 04u_Pa_14dpci 
 #       39815        291670         14945         35034         37650 
```
...
#length(AC_RE_VOL_SC[,1])
write.xlsx(AC_RE_VOL_SC,file=paste(PTHA1,"001",PROJECT,"_forSING_CELL_MATRIX.xlsx",sep=""),overwrite = T)
write.table (AC_RE_VOL_SC,file=paste(PTHA1,"001",PROJECT,"_forSING_CELL_MATRIX.txt",sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
write.table (MAT_MO1_u2,file=paste(PTHA1,"001",PROJECT,"_forSING_CELL_MATRIX_NO_SCALE.txt",sep=""),sep="\t",row.names = F,col.names=T,dec=".",quote = F)
...
```{r,fig.dim = c(8, 4),message=FALSE,class.source="bg-info",warning=FALSE}
...
```{r,fig.dim = c(9, 5),message=FALSE,class.source="bg-info",warning=FALSE}
print(arrange1t)
print(arrange1)
print(arrange1bt)
print(arrange1c)
...
FC0=0.09
PVLAUE_METH<- "bimod"
#smartseq2.markers_Wil70 <- FindAllMarkers(smartseq2, only.pos = TRUE, min.pct = "", logfc.threshold = FC0, test.use	= "wilcox") ## this also could be used
...
```{r,fig.dim = c(8, 9),message=FALSE,class.source="bg-info",warning=FALSE}
...
{pdf(file=paste(PTHA1,"08_", PROJECT,"Top5_",PP0,"per_incells_",FC0,"_FC_",PVLAUE_METH,".pdf",sep=""), width=10, height=10)  
...
P05ah_bino1<-DoHeatmap(smartseq2, features = top5will_bino$gene,group.colors=COe2) + NoLegend() + scale_fill_gradientn(colors = CO25) +  ggtitle(paste(PROJECT, "Top 5 marker",PP0,"per_incells_",FC0,"_FC_",PVLAUE_METH,sep=""))
...
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=F}
#VlnPlot(smartseq2, features = GENES001,pt.size = 0.3  ,group.by ='seurat_clusters',split.by = 'REG',    log = TRUE,cols = CO2b,  combine = F,sort = F)

 
#P04a <- VlnPlot(smartseq2, features = GENES001,pt.size = 0.3,group.by ='seurat_clusters',split.by = 'REG',log = TRUE,cols = CO2b, combine = F,sort = F)

#P04b <- VlnPlot(smartseq2, features = GENES002,pt.size = 0.3 ,group.by ='seurat_clusters',split.by = 'REG', log = TRUE,cols = CO2b, combine = F,sort = F)

#P04c <- VlnPlot(smartseq2, features = GENES003,pt.size = 0.3,group.by ='seurat_clusters',split.by = 'REG', log = TRUE,cols = CO2b, combine = F,sort = F)
#{pdf(file=paste(PTHA1,"07_", PROJECT,"CLU_vs_Exp_VIoPLOT_by_REG",".pdf",sep=""),width=8, height=2.5) 
#print(P04a)
#print(P04b)
#print(P04c)
#}
...
```{r,fig.dim = c(12, 8),message=FALSE,class.source="bg-info",warning=FALSE}
print(arrange12)
```
...
```{r,fig.dim = c(8, 8),message=FALSE,class.source="bg-info",warning=FALSE}
print(P05a1)
print(P05b1)
print(P05c1)
```
...
```{r,message=FALSE,class.source="bg-info",warning=FALSE,echo=F}
#{pdf(file=paste(PTHA1,"08_",PROJECT,"_CLU_vs_Ex_VIol_groups_RAW",".pdf",sep=""),width=15, height=6)  
#P04a1v <- VlnPlot(smartseq2, features = GENES001, slot = "counts", log = TRUE,cols=COe2)
#P04b1v <- VlnPlot(smartseq2, features = GENES002, slot = "counts", log = TRUE,cols=COe2)
#P04c1v <- VlnPlot(smartseq2, features = GENES003, slot = "counts", log = TRUE,cols=COe2)
#print(P04a1v)
#print(P04b1v)
#print(P04c1v)
#}
...
```{r,fig.dim = c(5, 5),message=FALSE,class.source="bg-info",warning=FALSE,echo=FALSE}
LE=10
SI=10
...
pdf(file=paste(PTHA1,"10_", PROJECT,"_CLU_vs_Ex_VIol_groups_STACKED",".pdf",sep=""), width=7, height=6)
print(P1)
print(arrange1CM)
print(arrange1CM3)
dev.off()
...
pdf(file=paste(PTHA1,"11_", PROJECT,"_CLU_vs_Ex_VIol_BY_groups_STACKED",".pdf",sep=""), width=7, height=8)
print(arrange1CM41)
print(arrange1CM42)
print(arrange1CM43)   
dev.off()
